...
class CUBEImportPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        layout.label(text="solid mode transparent thresholds (default 0, need restart)")
    ...
    def execute(self, context):
        ...
        def create_vdb(name, matname,dataf,framek):         
            ...
            if matname != "negative":
                ...
                vector_data[positive_values] = np.column_stack((colorval, 1-colorval, np.zeros_like(colorval)))
            else:
                colorval=dataf[negative_values]
                vector_data[negative_values] = np.column_stack((np.zeros_like(colorval) , 1-colorval, colorval))
